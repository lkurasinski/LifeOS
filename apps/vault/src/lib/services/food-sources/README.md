...
- **Implementation**: `$lib/domain/cookbook/foods/integrations/typesense/typesense.strategy.ts`
- **Data Source**: PostgreSQL database via Typesense search + Prisma
- **Use Case**: Search foods already in the database

### 2. FDC (FDCStrategy)
- **Name**: `'fdc'`
- **Implementation**: `$lib/domain/cookbook/foods/integrations/fdc/fdc-strategy.ts`
...
1. **Create integration** in `$lib/domain/cookbook/foods/integrations/{source}/`
...
2. **Register strategy** in `index.ts`:
   ```typescript
   import { NewStrategy } from '$lib/domain/cookbook/foods/integrations/new/new-strategy';
